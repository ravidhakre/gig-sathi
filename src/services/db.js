...
const OFFER_LETTER_WORD_TEMPLATE = `<div class="contract-page-sheet">
  ...
  <div class="contract-body">
    ...
    <p>The position of <strong>{{position}}</strong> is categorized as a Performance-Linked Customer Acquisition Role, wherein the Employee's overall contribution shall be evaluated on the basis of measurable business outcomes, operational efficiency, customer conversion ratio, compliance standards, and service quality. Accordingly, the Employee shall be entrusted with the responsibility of facilitating the successful activation of <strong>{{performance_target}}</strong> during every monthly evaluation cycle.</p>
    ...
    <p>The Employee shall be eligible for a Consolidated Monthly Remuneration of <strong>{{salary}}</strong>, subject to applicable deductions, Company policies, attendance compliance, operational discipline, and successful fulfillment of the prescribed Performance Evaluation Framework.</p>
...
export const dbService = {
  ...
  getTemplates: async () => {
    if (dbMode === 'FIREBASE') {
      try {
        ...
        if (templates.length > 0) {
          ...
          if (!cand || !cand.content || !cand.content.includes("ORGANIZATIONAL DISCIPLINE") || !cand.content.includes("Customer Relationship Executive")) {
            console.log('SRYN: Upgrading Firestore offer templates with 9-clause CRE Appointment Letter...');
            for (const temp of SEED_TEMPLATES) {
              await setDoc(doc(firebaseFirestore, 'templates', temp.id), temp);
            }
    ...
    if (!localCand || !localCand.content || !localCand.content.includes("ORGANIZATIONAL DISCIPLINE") || !localCand.content.includes("Customer Relationship Executive")) {
      localStorage.setItem('gs_templates', JSON.stringify(SEED_TEMPLATES));
      return SEED_TEMPLATES;
    }
